...
export function districtsFor(state: string): string[] {
  return INDIA_STATES.find((s) => s.name === state)?.districts ?? [];
}

export function matchState(raw: string): string {
  const q = fold(raw);
  if (!q) return '';
  const exact = INDIA_STATES.find((s) => fold(s.name) === q);
  if (exact) return exact.name;
  const fuzzy = INDIA_STATES.find((s) => fold(s.name).includes(q) || q.includes(fold(s.name)));
  return fuzzy?.name ?? '';
}
...
export function matchDistrict(state: string, raw: string): string {
  const q = fold(raw).replace(/\bdistrict\b/g, '').trim();
  if (!q) return '';
  const districts = districtsFor(state);
  const aliased = DISTRICT_ALIASES[q];
  if (aliased && districts.includes(aliased)) return aliased;
  const exact = districts.find((d) => fold(d) === q);
